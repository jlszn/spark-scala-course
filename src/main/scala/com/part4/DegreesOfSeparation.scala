 ...
 */
object DegreesOfSeparation extends Serializable {
  ...
  def convertToBFS(line: String): BFSNode = {
    ...
    val connections = for (connection <- 1 until fields.length) yield fields(connection).toInt
  ...
  def createStartingRdd(sc: SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("../../Downloads/SparkScala/Marvel-graph.txt")
