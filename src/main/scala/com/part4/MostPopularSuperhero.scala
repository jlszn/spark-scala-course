...
object MostPopularSuperhero extends Serializable {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("../../Downloads/SparkScala/Marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-apperarance data
    val lines = sc.textFile("../../Downloads/SparkScala/Marvel-graph.txt")
    ...
    val flipped: RDD[(Int, Int)] = totalFriendsByCharacter.map(x => (x._2, x._1) )
    ...
    val mostPopularName: String = namesRdd.lookup(mostPopular._2)(0)
